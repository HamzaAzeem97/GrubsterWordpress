 ...
 * Breadcrumbs trail.
 * 
 * Based on WooCommerce breadcrumbs.
 */
class Module
{
	protected $labels = [];
	...
	{	
		$defaults = array(
			'before'         => '<nav class="breadcrumbs"%1$s>',
			'after'          => '</nav>',
		...
		if (isset($options['labels'])) {
			$this->labels = array_merge($this->labels, (array) $options['labels']);
		}
		...
		if (function_exists('woocommerce_breadcrumb') && (is_woocommerce() || is_cart() || is_shop())) {
			...
			woocommerce_breadcrumb(array(
				...
				'before'      => '',
				'after'       => '',
				'wrap_before' => $options['before'] . $home_before,
				'wrap_after'  => $home_after . $options['after'],
